...
export class TablePrimengComponent implements OnChanges, OnInit {
  ...
  getColorEstado(rowData: any, rowIndex: number, tipo: 'check' | 'times'): string {
    const isActiveOrSelected = !!Number(rowData.bActive) || this.trSelected === rowIndex;
  ...
  @Output() accionBtnItem: EventEmitter<{ accion: any; item: any }> = new EventEmitter();
  ...
  onColumnSelected(columns) {
    ...
    if (this.columnsGroup) {
      ...
      const removedCols = this.columnas
        .filter(original => !columns.find(col => col.field === original.field))
        .map(col => col.col);

      // 2. Llamar a función con columnas originales y columnas eliminadas
      const { inTableColumns, inTableColumnsGroup } = this.removeColumnAndUpdateGroups({
  ...
  reAddRemovedCells(removedCols, inTableColumnsGroup) {
    ...
    const reAddedCells = removedCols.filter(col => {
      ...
      return !this.columnsGroup.some(group => group.some(cell => cell.col === col));
    });
  ...
  removeColumnAndUpdateGroups({ targetCols, inTableColumns, inTableColumnsGroup }) {
    ...
    const updatedGroups = inTableColumnsGroup.map(row => {
      return row
        .map(cell => {
          ...
          const remainingCols = range.filter(col => !targetCols.includes(col));
          ...
          const newCol = remainingCols[0] - targetCols.filter(t => t < remainingCols[0]).length;
   ...
   */
  mapTagStyles(row, col): 'success' | 'info' | 'warning' | 'danger' | 'secondary' | 'contrast' {
    if (col.styles === undefined) return 'secondary';
    const severity = ['success', 'info', 'warning', 'danger', 'secondary'].includes(
      col.styles[row[col.field]]
    );
