...
export class AsignarHorarioCapacitacionComponent implements OnInit {
  ...
  openDialog(dayKey: string) {
    ...
    this.activeDayName = this.dias.find(d => d.id === dayKey)?.nombre || '';
  ...
  getTotalClasses(): number {
    return Object.values(this.schedule).reduce((total, dayClasses) => total + dayClasses.length, 0);
  }
  ...
  anadirClase() {
    ...
    if (this.newClass.subject && this.newClass.startTime && this.newClass.endTime) {
      const classToAdd: ClassSchedule = {
        ...this.newClass,
        id: Date.now(),
      };

      this.schedule[this.activeDay as keyof WeeklySchedule].push(classToAdd);
