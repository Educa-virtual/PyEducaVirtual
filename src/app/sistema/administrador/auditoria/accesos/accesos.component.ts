...
export class AccesosComponent implements OnInit {
  ...
  getData(curr) {
    ...
    if (curr.filtroFecha[0] != null && curr.filtroFecha[1] != null) {
      this.auditoria
        .getData({
          filtroFechaInicio: this.utils.convertirAFechaSegura(this.form.value.filtroFecha[0]),
          filtroFechaFin: this.utils.convertirAFechaSegura(this.form.value.filtroFecha[1]),
        })
        .subscribe({
          next: res => {
