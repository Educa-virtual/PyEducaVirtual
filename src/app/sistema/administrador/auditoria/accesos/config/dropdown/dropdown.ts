    ...
    value: {
      ...
      response: (response: any) => {
        ...
        return response.data.map((item: any, index: number) => ({
          ...
          dtFecha: datePipe.transform(item.dtFecha, 'dd/MM/yyyy HH:mm'),
        }));
      },
    },
  },
    ...
    value: {
      ...
      response: (response: any) => {
        ...
        return response.data.map((item, index) => {
          const datosAntiguos = Array.isArray(JSON.parse(item.cAudDatosAntiguos))
            ? JSON.parse(item.cAudDatosAntiguos)[0]
            : {};
          const datosNuevos = Array.isArray(JSON.parse(item.cAudDatosNuevos))
          ...
          keys.forEach(key => {
            ...
            const entry = {
              class: isMatching ? undefined : item.cAudTipoOperaion,
              property: key,
          ...
          return {
            ...
            dtFecha: datePipe.transform(item.dtFecha, 'dd/MM/yyyy HH:mm'),
            cAudOperacion: JSON.parse(item.cAudOperacion)?.[0]['event_info'].replace(/,/g, ', '),
      ...
      response: (response: any) => {
        ...
        return response.data.map((item, index) => {
          ...
          keys.forEach(key => {
            ...
            const entry = {
              class: isMatching ? undefined : item.cAudTipoOperacion,
              property: key,
              oldValue: formatDataAntiguo ?? null,
              newValue: formatDataNuevo ?? null,
            };
