...
export class AreasListComponent implements OnInit {
  ...
  accionBtnItem(event: any) {
    ...
    switch (accion) {
      case 'agregar':
        this.titulo = 'Formulario para agregar área curricular (Curricula: ' + this.caption + ')';
      ...
      case 'actualizar_area':
        this.bUpdate = true;
        this.frmCursos.patchValue({ iEstado: 1 });
      ...
      case 'editar':
        this.titulo = 'Formulario para editar área curricular (Curricula: ' + this.caption + ')';
        this.frmCursos.reset();
        ...
        this.frmCursos.get('cCursoImagen')?.setValue(event.item.cCursoImagen);
        this.filesUrl = [];
  ...
  validarTotalCreditos() {
    ...
    if (!esValido) {
      this.messageService.add({
        ...
        detail: `Los créditos del área (${credito}) no pueden superar el total (${total_credito}) de créditos.`,
      });
    }
    this.frmCursos.patchValue({ vValidoCredito: esValido });
    return esValido;
  }
  ...
  validarTotalHoras() {
    ...
    this.frmCursos.patchValue({ vValidoHora: esValido });
    return esValido;
  }
  ...
  selectImage(image: any) {
    ...
    const data = { id: image.id, name: image.name, url: image.url };
    this.frmCursos.patchValue({ cCursoImagen: data.url });
  }
