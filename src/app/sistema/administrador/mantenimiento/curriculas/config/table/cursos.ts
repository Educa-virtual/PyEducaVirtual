...
export function accionBtnContainerCursos(this: CurriculasComponent, { accion }) {
...
export function cursosAccionBtnTable(this: CurriculasComponent, { accion, item }) {
  console.log(accion);
  ...
  switch (accion) {
    ...
    case 'assignCursosInNivelesGrados':
      Object.keys(this.forms.assignCursosInNivelesGrados.controls).forEach(field => {
        const control = this.forms.assignCursosInNivelesGrados.get(field);
