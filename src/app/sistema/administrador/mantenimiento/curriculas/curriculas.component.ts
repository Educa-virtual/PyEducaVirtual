...
import { assignCursosInNivelesGrados, editar, nivelesCursos } from './config/actions/table';
...
export class CurriculasComponent implements OnInit {
  ...
  showCursos() {
    ...
    this.cursosService.getCursos(this.forms.curriculas.value.iCurrId).subscribe({
      next: (res: any) => {
        console.log(res);
