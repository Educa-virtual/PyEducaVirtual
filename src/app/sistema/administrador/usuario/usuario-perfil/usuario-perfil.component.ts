...
export class UsuarioPerfilComponent implements OnChanges, OnInit {
  ...
  accionBtnItemTable({ accion, item }) {
    ...
    if (accion === 'eliminar_perfil') {
      ...
      this.btnItem('eliminar_perfil', '¿Desea eliminar el perfil seleccionado?');
    }
  }
      ...
      accept: () => {
        if (accion === 'perfil') {
          ...
          switch (this.iOptionId) {
            case 1: //DREMO
              const tipo = this.iDremoId == 1012 ? 'EspecialistaDre' : 'PerfilModuloDre';
        ...
        if (accion === 'eliminar_perfil') {
          ...
          this.eliminar_perfiles(this.selectedItemsPerfil['iCredEntPerfId']);
        }
      },
      reject: () => {
  ...
  getModulosAdministrativos() {
    this.query
      .searchCalAcademico({
        ...
        condicion: 'iModuloId=1012 and iModuloEstado=1 or iModuloId<10 and iModuloEstado=1',
      })
      .subscribe({
        next: (data: any) => {
        ...
        error: error => {
          console.error('Error lista de módulos Administrativos:', error);
          this.messageService.add({
            severity: 'danger',
            summary: 'Mensaje',
            detail: error,
  ...
  getInstitucionesEducativas() {
    this.query
      .searchCalAcademico({
        ...
        campos:
          'iIieeId, cIieeCodigoModular, cIieeNombre, cIieeRslCreacion, cIieeDireccion, cIieeLogo, iEstado, iNivelTipoId',
        condicion: 'iEstado=1',
      })
      .subscribe({
        next: (data: any) => {
    ...
    this.query
      .addCalAcademico({
        json: JSON.stringify({
          ...
          iPerfilId: this.form_perfil.value.iPerfilId,
          iPersId: this.usuario.iPersId,
          iCursosNivelGradId: this.form_perfil.value.iCursosNivelGradId,
          cTipo: tipo,
        }),
