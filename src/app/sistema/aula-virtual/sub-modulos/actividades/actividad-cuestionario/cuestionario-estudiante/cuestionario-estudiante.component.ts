...
export class CuestionarioEstudianteComponent implements OnChanges {
    ...
    obtenerPreguntaAlternativasRespuestasxiCuestionarioIdxiEstudianteId(
        iCuestionarioId
    ) {
                    ...
                    if (resp.validated) {
                        if (Number(resp.bCuestionarioActivo) !== 1) {
                            ...
                            this.preguntas.forEach((pregunta) => {
                                ...
                                if (
                                    typeof pregunta.jsonAlternativas ===
                                    'string'
                                ) {
                                    try {
                                ...
                                if (
                                    pregunta.cCodeTipoPreg === 'TIP-PREG-TEXTO'
                                ) {
                                    const respuestaSeleccionada =
                                        pregunta.jsonAlternativas.find(
                                            (alt) => alt.iPrgAltRptaId !== null
                                        )
                                    if (respuestaSeleccionada) {
                                        this.respuestasTexto[pregunta.iPregId] =
                                            respuestaSeleccionada.cRespuesta
                                    }
                                }

                                if (
                                    pregunta.cCodeTipoPreg ===
                                    'TIP-PREG-OPCIONES'
                                ) {
                                    const respuestaSeleccionada =
                                        pregunta.jsonAlternativas.find(
    ...
    guardarRespuesta(item: any, opcion: string, multiple): void {
        ...
        switch (item.cCodeTipoPreg) {
            ...
            case 'TIP-PREG-OPCIONES':
                const idOpcion = item.jsonAlternativas.find(
                    (alt) => alt.iPregAlterId === opcion
                )
                iPregAlterId = idOpcion.iPregAlterId
                break
            case 'TIP-PREG-CASILLA':
                const idMultiple = item.jsonAlternativas.find(
                    (alt) => alt.iPregAlterId === multiple
                )
                iPregAlterId = idMultiple.iPregAlterId
                break
            case 'TIP-PREG-DESPLEGABLE':
                const idDropdown = item.jsonAlternativas.find(
                    (alt) => alt.iPregAlterId === opcion
                )
