...
export class CuestionarioFormPreguntasComponent implements OnChanges {
  ...
  calif = [{ id: 3 }, { id: 4 }, { id: 5 }, { id: 6 }, { id: 7 }, { id: 8 }, { id: 9 }, { id: 10 }];
  icons = [
  ...
  obtenerCodigoTipoPregunta(even: any): void {
    const tipoPregunta = this.tipoPreguntas.find(t => t.iTipoPregId === even.value);
    if (tipoPregunta) {
  ...
  guardarPregunta() {
    ...
    if (
      ...
      alternativas.some(alt => !alt.cAlternativa || alt.cAlternativa.trim() === '')
    ) {
      this.messageService.add({
        severity: 'error',
        summary: 'Error de validación',
        detail: 'Debe agregar al menos 3 alternativa',
      });
