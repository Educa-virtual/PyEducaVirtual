...
@Component({
  ...
  providers: [DialogService, AulaBancoPreguntasService, provideIcons({ matWorkspacePremium })],
})
export class EvaluacionFormPreguntasComponent implements OnInit, OnDestroy {
  ...
  quitarPreguntaLocal(item) {
    if (item.preguntas == null) {
      this.preguntas = this.preguntas.filter(pregunta => pregunta.iEvalPregId != item.iEvalPregId);
    } else {
