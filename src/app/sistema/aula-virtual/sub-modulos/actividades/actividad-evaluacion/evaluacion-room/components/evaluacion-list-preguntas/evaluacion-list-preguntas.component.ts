...
export class EvaluacionListPreguntasComponent implements OnChanges {
  ...
  ngOnChanges(changes) {
    if (changes.preguntas.currentValue) {
      ...
      this.preguntas.forEach(pregunta => {
        ...
        if (pregunta.jsonPreguntas && typeof pregunta.jsonPreguntas.jsonAlternativas === 'string') {
          try {
  ...
  mostrarListaAlternativas(item: any): boolean {
    return Array.isArray(item.jsonAlternativas) && item.jsonAlternativas.length > 0;
  }
    ...
    this._ConfirmationModalService.openConfirm({
      ...
      accept: () => {
        idEncabPregId
          ...
          : iEvalPregId && this.eliminarEvaluacionPreguntas(iEvalPregId);
      },
      reject: () => {
