...
export class ForoRoomComponent extends MostrarErrorComponent implements OnInit {
        ...
        next: resp => {
          this.foro = resp.length
            ? {
                ...
                cDocumentos: resp[0].cForoUrl ? JSON.parse(resp[0].cForoUrl) : [],
                cTitle: 'Foro: ' + (resp[0]?.cForoTitulo || 'Título no disponible'),
          ...
          this.FilesTareas = this.foro?.cForoUrl ? JSON.parse(this.foro?.cForoUrl) : [];
    ...
    this._aulaService
      ...
      .subscribe({
        next: (resp: Record<string, any>) => {
          this.respuestasForo = Object.values(resp).map(comment => ({
            ...comment,
            expanded: false,
          }));
          this.data = resp['data'];
          ...
          this.respuestasForo.forEach(respuesta => {
            respuesta.json_respuestas_comentarios = respuesta.json_respuestas_comentarios
              ? JSON.parse(respuesta.json_respuestas_comentarios)
              : [];
            // Sumar comentarios principales + anidados
            this.totalComentarios += respuesta.json_respuestas_comentarios.length;
          });
