...
@Component({
  ...
  standalone: true,
  imports: [TareaFormComponent, PrimengModule],
...
una tarea o actividad a un servicio backend, con la finalidad de guardarla o actualizarla en la base de datos.*/
  submitFormulario(data) {
    ...
    if (this.action === 'GUARDAR') {
      data.iContenidoSemId = this.dialogConfig.data.contenidoSemana.iContenidoSemId;
  ...
  getTareasxiTareaId(iTareaId) {
    ...
    this._generalService.getGralPrefix(params).subscribe({
      next: resp => {
        this.tarea = resp.data.length ? resp.data[0] : this.closeModal(resp.validated);
      },
    });
  }
}
