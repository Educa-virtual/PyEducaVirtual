...
@Component({
  ...
  imports: [PrimengModule, CommonInputComponent, InputTextModule, InputGroupModule],
})
export class VideoconferenciaFormContainerComponent implements OnInit, OnChanges {
  ...
  guardar() {
    ...
    if (this.formConferencia.invalid) {
      ...
      switch (this.action) {
        case 'ACTUALIZAR':
          this.apiAulaService
            .actualizarReunionVirtual(data, this.actividad.ixActivadadId)
            .subscribe({
              next: (response: any) => {
