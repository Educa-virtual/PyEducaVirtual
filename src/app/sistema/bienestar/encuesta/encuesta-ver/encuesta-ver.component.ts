...
export class EncuestaVerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.puede_editar) {
      ...
      this.breadCrumbItems = [
        ...
        {
          label: 'Respuestas',
          routerLink: '/bienestar/encuesta/' + this.iEncuId + '/respuestas',
        },
        {
          label: 'Responder encuesta',
        },
  ...
  setRespuestasFormArray(respuestas: any[]) {
    ...
    preguntasFA.controls.forEach((preguntaFG: FormGroup) => {
      const respuesta = respuestas.find(
        (respuesta: any) => respuesta.iEncuPregId == preguntaFG.get('iEncuPregId')?.value
      );
      if (respuesta) {
  ...
  guardarRespuesta() {
    ...
    this.datosEncuestas.guardarRespuesta(this.formPreguntas.value).subscribe({
      next: () => {
        this._messageService.add({
          severity: 'success',
  ...
  actualizarRespuesta() {
    ...
    this.datosEncuestas.actualizarRespuesta(this.formPreguntas.value).subscribe({
      next: () => {
        this._messageService.add({
          severity: 'success',
