...
export class EncuestaComponent implements OnInit {
  ...
  actualizarEncuesta() {
    ...
    this.datosEncuestas.actualizarEncuesta(this.formEncuesta.value).subscribe({
      next: () => {
        this._messageService.add({
          severity: 'success',
          summary: 'Registro exitoso',
  ...
  obtenerPoblacionObjetivo() {
    ...
    this.datosEncuestas
      ...
      .subscribe({
        next: (data: any) => {
          if (data.data.length) {
            this.cantidad_poblacion = data.data[0].iPoblacionObjetivo;
          } else {
  ...
  accionBtnItemTablePermisos({ accion, item }) {
    if (accion == 'eliminar') {
      this.permisos = this.permisos.filter((permiso: any) => item.iEncuPobId != permiso.iEncuPobId);
    }
  }
