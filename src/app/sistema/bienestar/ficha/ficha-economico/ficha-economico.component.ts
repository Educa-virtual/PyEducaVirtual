...
@Component({
  ...
  standalone: true,
  imports: [PrimengModule, InputSimpleComponent, DropdownSimpleComponent, SwitchSimpleComponent],
...
export class FichaEconomicoComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.datosFichaBienestar.getFichaParametros().subscribe((data: any) => {
      this.rangos_sueldo = this.datosFichaBienestar.getRangosSueldo(data?.rangos_sueldo);
      this.dependencias_economicas = this.datosFichaBienestar.getDependenciasEconomicas(
        data?.dependencias_economicas
      );
      this.tipos_apoyo_economico = this.datosFichaBienestar.getTiposApoyoEconomico(
        data?.tipos_apoyo_economico
      );
      this.jornadas_trabajo = this.datosFichaBienestar.getJornadasTrabajo(data?.jornadas_trabajo);
    });
  ...
  actualizar() {
    ...
    this.datosFichaBienestar.actualizarFichaEconomico(this.formEconomico.value).subscribe({
      next: (data: any) => {
        this.formEconomico.get('iIngresoEcoId').setValue(data.data[0].iIngresoEcoId);
        this._MessageService.add({
          severity: 'success',
        ...
        setTimeout(() => {
          this.router.navigate([`/bienestar/ficha/${this.iFichaDGId}/vivienda`]);
        }, 1000);
      },
      error: error => {
