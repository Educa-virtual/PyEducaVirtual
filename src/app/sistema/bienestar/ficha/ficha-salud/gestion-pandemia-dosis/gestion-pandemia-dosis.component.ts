...
export class GestionPandemiaDosisComponent implements OnInit {
  ...
  filtrarTabla() {
    ...
    this.dosis_filtradas = this.dosis.filter((dosis: any) => {
      if (dosis.iPanDFichaNroDosis.toLowerCase().includes(filtro)) return dosis;
      if (dosis.dtPanDFichaDosis.toLowerCase().includes(filtro)) return dosis;
      if (dosis.cPandemiaNombre.toLowerCase().includes(filtro)) return dosis;
      return null;
    });
  }
