...
@Component({
  ...
  standalone: true,
  imports: [PrimengModule, DropdownInputComponent, MultiselectInputComponent, InputSimpleComponent],
...
export class FichaViviendaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.datosFichaBienestar.getFichaParametros().subscribe((data: any) => {
      this.ocupaciones_vivienda = this.datosFichaBienestar.getOcupacionesVivienda(
        data?.ocupaciones_vivienda
      );
      this.estados_vivienda = this.datosFichaBienestar.getEstadosVivienda(data?.estados_vivienda);
      this.tipos_vivienda = this.datosFichaBienestar.getTiposVivienda(data?.tipos_vivienda);
      this.materiales_paredes_vivienda = this.datosFichaBienestar.getParedesVivienda(
        data?.materiales_paredes_vivienda
      );
      this.materiales_techos_vivienda = this.datosFichaBienestar.getTechosVivienda(
        data?.materiales_techos_vivienda
      );
      this.materiales_pisos_vivienda = this.datosFichaBienestar.getPisosVivienda(
        data?.materiales_pisos_vivienda
      );
      this.suministros_agua = this.datosFichaBienestar.getSuministrosAgua(data?.suministros_agua);
      this.tipos_sshh = this.datosFichaBienestar.getTiposSshh(data?.tipos_sshh);
      this.tipos_alumbrado = this.datosFichaBienestar.getTiposAlumbrado(data?.tipos_alumbrado);
      this.otros_elementos = this.datosFichaBienestar.getOtrosElementos(data?.otros_elementos);
    });
  ...
  actualizar() {
    ...
    this.datosFichaBienestar.actualizarFichaVivienda(this.formVivienda.value).subscribe({
      next: (data: any) => {
        this.formVivienda.get('iViendaCarId').setValue(data.data[0].iViendaCarId);
        this.ficha_registrada = true;
        this._messageService.add({
          severity: 'success',
        ...
        setTimeout(() => {
          this.router.navigate([`/bienestar/ficha/${this.iFichaDGId}/alimentacion`]);
        }, 1000);
      },
      error: error => {
