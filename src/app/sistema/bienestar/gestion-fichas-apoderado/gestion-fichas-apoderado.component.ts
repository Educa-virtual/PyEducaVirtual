...
export class GestionFichasApoderadoComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.datosFichaBienestar
            .listarEstudiantesApoderado({
                iCredEntPerfId: this.iCredEntPerfId,
                iYAcadId: this.iYAcadId,
            })
            .subscribe({
                next: (response: any) => {
                    this.estudiantes = response.data.map(
                        function (item, index) {
                            item.index = index + 1
                            item.dtFichaDGFormateada = formatDate(
                                item.dtFichaDG,
                                'dd/MM/yyyy',
                                'en-US'
                            )
                ...
                error: (error) => {
                    ...
                    this._messageService.add({
                        ...
                        summary: 'Error',
                        detail:
                            'No se pudo cargar los estudiantes: ' +
                            error.message,
                    })
                },
            })
        ...
        switch (accion) {
            ...
            case 'editar':
                this.router.navigate([
                    `/bienestar/ficha-declaracion/${item.iPersId}`,
                ])
                break
            case 'eliminar':
