...
export class RecordatorioFechasComponent implements OnInit {
  ...
  filtrarTabla() {
    ...
    this.cumpleanios_filtrados = this.cumpleanios.filter((cumple: any) => {
      if (cumple.iCumpleaniosDiff.toLowerCase().includes(filtro.toLowerCase())) {
        return cumple;
      }
      if (cumple.cCumpleaniosFormateado.toLowerCase().includes(filtro.toLowerCase())) {
        return cumple;
      }
      if (cumple.cPersNombreApellidos.toLowerCase().includes(filtro.toLowerCase())) {
        return cumple;
      }
      if (cumple.cRelacionNombre.toLowerCase().includes(filtro.toLowerCase())) {
        return cumple;
      }
      return null;
    });
