...
@Component({
  ...
  standalone: true,
  imports: [PrimengModule, FormsModule, TabViewModule, EditorComponent, TablePrimengComponent],
...
export class ContenidosComponent implements OnInit {
  ...
  getCamposMostrables(
    ...
    return campos.filter(campo => campo.value != null && campo.value.trim() !== '');
  }
  ...
  editComunicado(com: Comunicado) {
    this.selectedComunicado = {
      ...
      prioridad:
        this.prioridades.find((p: any) => p.label === com.prioridad)?.value || com.prioridad,
      tipo: this.tipos.find((t: any) => t.label === com.tipo)?.value || com.tipo,
  ...
  guardarComunicado() {
    ...
    if (!this.selectedComunicado.titulo?.trim()) {
      this.mensajesEmergentes('warn', 'Advertencia', 'Debe ingresar un Título');
      return;
    }
    if (!this.selectedComunicado.prioridad) {
      this.mensajesEmergentes('warn', 'Advertencia', 'Debe seleccionar Prioridad');
      return;
    }
    if (!this.selectedComunicado.tipo) {
      this.mensajesEmergentes('warn', 'Advertencia', 'Debe seleccionar Tipo');
      return;
    }
    // Validar que se ingresen ambas fechas
    if (!this.selectedComunicado.publicado || !this.selectedComunicado.caduca) {
      this.mensajesEmergentes('warn', 'Advertencia', 'Debe ingresar la fecha de Inicio y Fin.');
      return;
    }
    ...
    switch (accion) {
      case 'obtenerDatos':
        ...
        if (Number(this.iPerfilId) === 7) {
          this.tipos = this.tipos.filter(t => t.value === null || Number(t.value) === 2);
        }
        break;
      case 'obtenerComunicadosPersona':
        // item es el array devuelto por el SP
        this.comunicados = item.map((c: any) => {
          const foundCurso = this.cursos.find((cs: any) => cs.value == c.iCursoId);
          const foundSeccion = this.secciones.find((ss: any) => ss.value == c.iSeccionId);
          const foundGrado = this.grados.find((gg: any) => gg.value == c.iGradoId);
          ...
          return {
            ...
            estado: Number(c.bComunicadoArchivado) === 1 ? 'Activo' : 'Inactivo',
            // Nombres
            ...
            publicado: c.dtComunicadoEmision
              ...
              : '',
            caduca: c.dtComunicadoHasta ? this.formatDate(new Date(c.dtComunicadoHasta)) : '',
            ...
            cursoName: c.iCursoId !== null && foundCurso ? foundCurso.label : null,
            seccionName: c.iSeccionId !== null && foundSeccion ? foundSeccion.label : null,
            gradoName: c.iGradoId !== null && foundGrado ? foundGrado.label : null,
  ...
  aplicarLogicaEnvio() {
    if (!this.advancedOptions) {
      ...
      if (!this.selectedComunicado.grupo || this.selectedComunicado.grupo.length === 0) {
        this.messageService.add({
          severity: 'warn',
          summary: 'Advertencia',
  ...
  seleccionarDocente(event: any) {
    ...
    this.docenteNombre =
      docente.cDocenteNombres + ' ' + docente.cDocentePaterno + ' ' + docente.cDocenteMaterno;
