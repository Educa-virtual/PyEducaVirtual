...
import { TablePrimengComponent, IColumn } from '@/app/shared/table-primeng/table-primeng.component';
...
export class InformesComponent implements OnInit {
    ...
    this.mensaje = [
      {
        severity: 'info',
        detail:
          'En esta sección podrá visualizar sus logros alcanzados del estudiantes durante el año escolar',
      },
    ];
  ...
  obtenerEstudianteXCurso() {
    ...
    this._aulaService
      ...
      .subscribe(data => {
        ...
        console.log('Estudiantes x Grado ', this.estudianteMatriculadosxGrado);
      });
  }
  ...
  guardarPromedioDeLogroAlcanzado() {
    ...
    const nombre = this.estudianteSelect['Estudiante'];
    const conclusionDescrpLimpia = this.limpiarHTML(conclusionDescrp.conclusionDescripFinal);
    ...
    if (!this.estudianteSelect || this.estudianteSelect.length === 0) {
      ...
      if (conclusionDescrpLimpia == '') {
        ...
        this._confirmService.openConfiSave({
          ...
          accept: () => {
            this._aulaService
              .guardarCalificacionEstudiante('acad', 'matricula', where, registro)
              .subscribe({
                next: response => {
