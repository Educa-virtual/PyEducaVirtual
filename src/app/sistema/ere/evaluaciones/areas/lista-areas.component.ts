...
export class ListaAreasComponent implements OnInit {
  ...
  public onFilter(dv: DataView, event: Event) {
    ...
    if (text.length > 1) {
      this.cursos = this.cursos.filter(curso => normalizeText(curso.cCursoNombre).includes(text));
    }
  }
  ...
  actualizarEstadoArchivoSubido(datos: { curso: ICurso }) {
    ...
    this.gestionarPreguntasCard?.forEach(card => {
      if (card.curso.iCursosNivelGradId === datos.curso.iCursosNivelGradId) {
        card.curso.bTieneArchivo = true;
      }
    });
  }
  ...
  obtenerAreasPorEvaluacion() {
    this.areasService.obtenerAreasPorEvaluacion(this.iEvaluacionIdHashed).subscribe({
      next: respuesta => {
