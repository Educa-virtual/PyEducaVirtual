...
export class SimpleListaAreasComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['iEvaluacionIdHashed']?.currentValue) {
      this.iEvaluacionIdHashed = changes['iEvaluacionIdHashed'].currentValue;
    }
    if (changes['cursosFromParent']?.currentValue) {
      this.cursosFromParent = changes['cursosFromParent'].currentValue;
      this.cursos = [...this.cursosFromParent];
  ...
  descargarArchivoPreguntasPorArea(tipoArchivo: string): void {
    ...
    this.evaluacionesService.descargarArchivoPreguntasPorArea(params).subscribe({
      next: (response: Blob) => {
  ...
  descargarMatrizPorEvaluacionArea(): void {
    ...
    this.evaluacionesService.descargarMatrizPorEvaluacionArea(params).subscribe({
      next: (response: Blob) => {
        const url = window.URL.createObjectURL(response);
        const a = document.createElement('a');
        a.href = url;
    ...
    switch (tipo) {
      case 'Cuadernillo':
        if (curso.bTieneArchivo) {
          ...
          alert('No se ha subido un archivo de cuadernillo para esta área.');
        }
        break;
  ...
  eliminarArchivoCuadernillo(curso: ICurso): void {
    ...
    this.confirmationService.confirm({
      ...
      acceptButtonStyleClass: 'p-button-success p-button-sm custom-accept',
      rejectButtonStyleClass: 'p-button-danger p-button-sm custom-reject',
      accept: () => {
        this.areasService
          .eliminarArchivoPreguntasPdf(curso.iEvaluacionIdHashed, curso.iCursosNivelGradId)
          .subscribe({
            next: () => {
            ...
            error: err => {
              this.messageService.add({
                severity: 'error',
                summary: 'Error',
                detail: err.error.message || 'Error al eliminar el archivo de cuadernillo.',
              });
            },
          });
      },
  ...
  private agruparCursosPorGrado(): void {
    ...
    this.cursos.forEach(curso => {
      const grado = curso.cGradoAbreviacion?.toString().substring(0, 1) || '0';
      const nivel = curso.cNivelTipoNombre?.toString().replace('Educación ', '') || 'Sin nivel';
  ...
  obtenerIndiceGlobal(curso: ICurso): number {
    return this.cursos.findIndex(c => c.iCursosNivelGradId === curso.iCursosNivelGradId) + 1;
  }
