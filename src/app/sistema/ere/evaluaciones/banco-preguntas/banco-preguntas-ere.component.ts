...
export class BancoPreguntasComponent implements OnInit {
  ...
  @Output() actualizarListaPreguntasEvent: EventEmitter<boolean> = new EventEmitter();
      ...
      case 'setearDataxseleccionado':
        if (item.seleccionado) {
          this.preguntasSeleccionadas.push({
            ...
            cTipoPregDescripcion: item.iEncabPregId == null ? 'unica' : 'multiple',
          });
        } else {
          this.preguntasSeleccionadas = this.preguntasSeleccionadas.filter(
  ...
  obtenerPreguntas() {
    ...
    params = params.set('tipo_pregunta', this.formCriterios.get('ddTipoPregunta')?.value);

    if (this.formCriterios.get('ddCompetencia')?.value != null) {
      params = params.set('competencia', this.formCriterios.get('ddCompetencia')?.value);
    }
    if (this.formCriterios.get('ddAnioEvaluacion')?.value != null) {
      params = params.set('anio_evaluacion', this.formCriterios.get('ddAnioEvaluacion')?.value);
    }
    if (this.formCriterios.get('ddNivelEvaluacion')?.value != null) {
      params = params.set('nivel_evaluacion', this.formCriterios.get('ddNivelEvaluacion')?.value);
    }
    if (this.formCriterios.get('ddCapacidad')?.value != null) {
      params = params.set('capacidad', this.formCriterios.get('ddCapacidad')?.value);
    }
    this.preguntasService
      .obtenerPreguntas(this.iEvaluacionId, this.iCursosNivelGradId, params)
      .subscribe({
        next: respuesta => {
