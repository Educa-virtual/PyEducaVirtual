...
export class PreguntasComponent implements OnInit {
  ...
  mostrarVistaPrevia() {
    this.router.navigate([
      `ere/evaluaciones/${this.iEvaluacionId}/areas/${this.iCursoNivelGradId}/preguntas/vista-previa`,
    ]);
  }
  ...
  obtenerCantidadMaximaPreguntas() {
    this._apiEre
      .obtenerCantidadMaximaPreguntas(this.iEvaluacionId, this.iCursoNivelGradId)
      .subscribe({
        next: (resp: any) => {
  ...
  confirmarEliminarPregunta(pregunta) {
    ...
    const cabeceraPregunta = truncateText.transform(removeHtml.transform(pregunta.title), 40);
    ...
    this._ConfirmationModalService.openConfirm({
      ...
      accept: () => {
        if (Object.prototype.hasOwnProperty.call(pregunta, 'pregunta')) {
  ...
  handleNuevaPreguntaConEnunciadoSinData(item) {
    ...
    const preguntasConEncabezado = this.preguntas.filter(p => p.iEncabPregId === item.iEncabPregId);
    ...
    if (item.iEncabPregId && item.cEncabPregContenido) {
      this.enunciadosCache.set(item.iEncabPregId, item.cEncabPregContenido);
    }
  ...
  guardarPreguntaSinEnunciadoSinData() {
    ...
    const params = {
      ...
      data: {
        ...
        bPreguntaEstado: 1,
        cPreguntaClave: null,
        iEspecialistaId: this._ConstantesService.iEspecialistaId || null,
  ...
  guardarPreguntaConData(encabezado, pregunta, contenido) {
    ...
    const data = !encabezado ? (preguntas.length ? preguntas[0] : null) : pregunta;
    let error = false;
    data.opcion = !encabezado ? 'ACTUALIZARxiDesempenoId' : 'ACTUALIZARxiEncabPregId';
  ...
  cambiarEstadoCheckbox(iAlternativaId, alternativas) {
    ...
    const index = alternativas.findIndex(
      alt =>
        (alt.iAlternativaId !== null && alt.iAlternativaId == iAlternativaId) ||
        (alt.iAlternativaId === null && alt === alternativas[iAlternativaId])
    );
  ...
  accionBtnItem(elemento): void {
    ...
    switch (accion) {
      case 'CONSULTARxiEvaluacionIdxiCursoNivelGradId':
        ...
        this.isSecundaria = this.data.iNivelTipoId === '4' ? true : false;
        this.preguntas = [];
        const evaluaciones = item.length ? JSON.parse(item[0]['evaluaciones']) : [];
        ...
        for (const key in evaluaciones) {
          if (evaluaciones[key]['iCompetenciaId']) {
            evaluaciones[key]['iCompetenciaId'] = evaluaciones[key]['iCompetenciaId'].toString();
          }
          if (evaluaciones[key]['iCapacidadId']) {
            evaluaciones[key]['iCapacidadId'] = evaluaciones[key]['iCapacidadId'].toString();
          }
        ...
        this.preguntas.forEach(pregunta => {
          {
            if (pregunta.pregunta.length) {
              pregunta.pregunta.forEach(item => {
                ...
                if (item.iEncabPregId != null) {
                  const preguntaMultiple = this.preguntas.find(
                    o => o.iEncabPregId == item.iEncabPregId
                  );
                  if (preguntaMultiple) {
                    preguntaMultiple.title += ' #' + this.totalPregunta;
                  }
                }
            ...
            if (pregunta.pregunta.length > 1) {
              ...
              pregunta['iCompetenciaId'] = pregunta.pregunta[0]['iCompetenciaId'];
              pregunta['iCapacidadId'] = pregunta.pregunta[0]['iCapacidadId'];
              pregunta['cDesempenoDescripcion'] = pregunta.pregunta[0]['cDesempenoDescripcion'];
        ...
        this.preguntas.forEach(pregunta => {
          if (pregunta.iEncabPregId && this.enunciadosCache.has(pregunta.iEncabPregId)) {
            // Restauramos el contenido desde el caché
            pregunta.cEncabPregContenido = this.enunciadosCache.get(pregunta.iEncabPregId);
            ...
            if (pregunta.pregunta && pregunta.pregunta.length) {
              pregunta.pregunta.forEach(subPregunta => {
                subPregunta.cEncabPregContenido = pregunta.cEncabPregContenido;
              });
            }
          }
        });
  ...
  filtrarCapacidades(item): any[] {
    return this.matrizCapacidad.filter(i => i.iCompetenciaId === item.iCompetenciaId);
  }

  obtenerIdCompetenciaPorIdCapacidad(iCapacidadId: any): any {
    const capacidad = this.matrizCapacidad.find(cap => cap.iCapacidadId === iCapacidadId);
  ...
  async uploadImageFile(file: File, alternativa: any) {
    ...
    this.http.post(`${this.backendApi}/general/subir-archivo`, dataFile).subscribe({
      next: (response: any) => {
