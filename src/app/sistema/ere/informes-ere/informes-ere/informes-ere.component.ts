...
export class InformesEreComponent implements OnInit {
  ...
  searchResultados() {
    ...
    this.datosInformes.obtenerInformeResumen(this.formFiltros.value).subscribe({
      next: (data: any) => {
  ...
  mostrarEstadisticaNivel() {
    ...
    const niveles_nombres = this.niveles.map((item: any) => item.nivel_logro);
    const niveles_valores = this.niveles.map((item: any) => Number(item.cantidad));
  ...
  mostrarEstadisticaPregunta() {
    ...
    const preguntas = [];
    const aciertos = [];
    const fila_aciertos = this.resumen.findIndex((item: any) => item.metrica == '% DE ACIERTOS');
    const desaciertos = [];
    const fila_desaciertos = this.resumen.findIndex(
      (item: any) => item.metrica == '% DE DESACIERTOS'
    );
    const blancos = [];
    const fila_blancos = this.resumen.findIndex((item: any) => item.metrica == '% DE BLANCOS');
  ...
  mostrarPromedio(niveles, valores) {
    ...
    const porcentajes = valores.map(valor => ((Number(valor) / Number(total)) * 100).toFixed(2));
    const data = [];
