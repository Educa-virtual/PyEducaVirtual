...
export class RecursosComponent implements OnInit {
  ...
  filtrarRecursos() {
    if (!this.terminoBusqueda || this.terminoBusqueda.trim() === '') {
      ...
      this.datosRecursos4toFiltrados = this.recursos4to.filter(recurso =>
        recurso.area.toLowerCase().includes(termino)
      );

      this.datosRecursos6toFiltrados = this.recursos6to.filter(recurso =>
        recurso.area.toLowerCase().includes(termino)
      );
    }
  }
  ...
  guardarEdicion() {
    ...
    setTimeout(() => {
      ...
      const recursoOriginal = this.buscarRecursoOriginal(this.recursoEditando!.id);
