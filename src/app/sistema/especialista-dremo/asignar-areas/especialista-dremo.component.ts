...
export class AsignarAreasEspecialistaDremoComponent implements OnInit {
  ...
  obtenerCursosxNivel() {
    forkJoin({
      ...
      next: respuesta => {
        ...
        this.gradosSecundaria = this.agruparPorGrado(this.nivelSecundaria);
  ...
  obteneriCursosNivelGradIdxiDocenteId() {
    ...
    this._ApiEspecialistasService.obtenerAreasPorEspecialista(this.iDocenteId).subscribe({
      next: respuesta => {
        this.iAreasConAsignar = respuesta.length;
        this.iAreasSinAsignar =
          this.nivelPrimaria.length + this.nivelSecundaria.length - this.iAreasConAsignar;

        this.nivelPrimaria.forEach(nivelSinAsignar => {
          nivelSinAsignar.isSelected = false;
          respuesta.forEach(nivelConAsignar => {
            if (nivelSinAsignar.iCursosNivelGradId === nivelConAsignar.iCursosNivelGradId) {
  ...
  checkedCurso(item) {
    if (!this.iDocenteId) {
      this.mostrarMensaje('error', 'Error', 'No ha seleccionado un docente');
      return;
    }
    if (item.isSelected) {
  ...
  insertarCursos(curso) {
    ...
    this._ApiEspecialistasService.asignarAreaEspecialista(this.iDocenteId, data).subscribe({
      next: respuesta => {
        this.mostrarMensaje('success', 'Guardado!', respuesta.message);
  ...
  eliminarCursos(curso) {
    ...
    this._ApiEspecialistasService.eliminarAreaEspecialista(this.iDocenteId, data).subscribe({
      next: respuesta => {
        this.mostrarMensaje('success', 'Eliminado!', respuesta.message);
        this.iAreasConAsignar--;
        this.iAreasSinAsignar++;
      },
      error: err => {
