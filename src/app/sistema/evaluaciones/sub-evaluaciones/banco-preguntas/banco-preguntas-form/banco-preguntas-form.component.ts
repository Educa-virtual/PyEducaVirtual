...
export class BancoPreguntasFormComponent implements OnInit, OnDestroy {
  ...
  set pregunta(pregunta) {
    ...
    if (pregunta.preguntas !== undefined && pregunta.preguntas?.length > 0) {
  ...
  ngOnInit() {
    ...
    this.bancoPreguntasForm.get('0.encabezadoSelected').valueChanges.subscribe(value => {
      ...
      if (typeof value === 'string') {
        ...
        this.bancoPreguntasForm.get('0.cEncabPregTitulo').setValue(value);
      }
  ...
  agregarQuitarValidacionesEncabezado(mode: 'AGREGAR' | 'QUITAR') {
    if (mode === 'AGREGAR') {
      this.formEncabezado.get('iEncabPregId').setValidators([Validators.required]);
      this.formEncabezado.get('cEncabPregTitulo').setValidators([Validators.required]);
      this.formEncabezado.get('cEncabPregContenido').setValidators([Validators.required]);
    } else {
  ...
  agregarQuitarValidacionesFormPregunta(mode: 'AGREGAR' | 'QUITAR') {
    if (mode === 'AGREGAR') {
      this.formPreguntaInfo.get('iPreguntaId').setValidators([Validators.required]);
      this.formPreguntaInfo.get('iTipoPregId').setValidators([Validators.required]);
      this.formPreguntaInfo.get('cPregunta').setValidators([Validators.required]);
      this.formPreguntaInfo.get('iPreguntaNivel')?.setValidators([Validators.required]);
      this.formPreguntaInfo.get('iPreguntaPeso').setValidators([Validators.required]);
      this.formPreguntaInfo.get('iHoras').setValidators([Validators.required]);
      this.formPreguntaInfo.get('iMinutos').setValidators([Validators.required]);
      this.formPreguntaInfo.get('iSegundos').setValidators([Validators.required]);
      ...
      Object.keys(this.formPreguntaInfo.controls).forEach(key => {
        ...
        this.bancoPreguntasForm.get('1').get(key).updateValueAndValidity();
      });
    }
  }
  ...
  get esSinEncabezado(): boolean {
    return (
      typeof this.bancoPreguntasForm.get('0.encabezadoSelected').value === 'object' &&
      this.bancoPreguntasForm.get('0.encabezadoSelected').value?.iEncabPregId === -1
    );
  }
  ...
  goStep(opcion: string) {
    switch (opcion) {
      case 'next':
        if (this.bancoPreguntasForm.get(this.activeIndex.toString()).invalid) {
      ...
      case 'back':
        if (this.activeIndex !== 0) {
          if (this.formMode === 'SUB-PREGUNTAS' && this.pasos[this.activeIndex].id === '2') {
            this.agregarQuitarAlternativasPaso('AGREGAR');
          }
          this.activeIndex--;
        }
  ...
  handleTipoPreguntaChange(): void {
    const alternativasControl = this.bancoPreguntasForm.get('1.alternativas');
  ...
  eliminarPreguntaLocal(pregunta) {
    this.preguntas = this.preguntas.filter(item => item.iPreguntaId != pregunta.iPreguntaId);
  }
  ...
  actualizarPreguntas(pregunta) {
    ...
    if (existePregunta) {
      const index = this.preguntas.findIndex(x => x.iPreguntaId == pregunta.iPreguntaId);
  ...
  obtenerTipoPreguntaDesc(iTipoPregId: number) {
    return this.tipoPreguntas.find(item => item.iTipoPregId === iTipoPregId);
  }
  ...
  guardarActualizarBancoPreguntas() {
    ...
    if (this.formMode === 'UNA-PREGUNTA') {
      ...
      preguntas = [pregunta];
      const { isValid, message } = validarPregunta(pregunta, this.alternativas);
    ...
    if (this.formMode === 'SUB-PREGUNTAS') {
      ...
      for (const subPregunta of preguntas) {
        const { isValid, message } = validarPregunta(subPregunta, this.alternativas);

        if (!isValid) {
          this._confirmationModalService.openAlert({
            header: subPregunta.cPregunta.replace(/<\/?[^>]+(>|$)/g, ''),
            message,
          });
          return;
        }
  ...
  getPreguntaClave(alternativas = []) {
    return alternativas.find(alt => alt.bAlternativaCorrecta)?.cAlternativaLetra;
  }
