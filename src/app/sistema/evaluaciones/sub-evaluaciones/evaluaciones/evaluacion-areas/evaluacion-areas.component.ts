...
export class EvaluacionAreasComponent implements OnDestroy, OnInit {
  ...
  eliminarCursos(cursos: any[]): void {
    ...
    if (!this._iEvaluacionId || !cursos || cursos.length === 0) {
      console.error('Datos incompletos para eliminar cursos.', this._iEvaluacionId);
      this._MessageService.add({
        severity: 'error',
        summary: 'Error',
              ...
              Object.keys(nivel.grados).forEach(grado => {
                nivel.grados[grado].forEach((curso: any) => {
                  ...
                  const fechaStr = this.data.find(
                    i => i.iCursoNivelGradId == curso.iCursoNivelGradId
                  )?.dtExamenFechaInicio;
                  if (fechaStr) {
                  ...
                  curso.iExamenCantidadPreguntas = this.data.find(
                    i => i.iCursoNivelGradId === curso.iCursoNivelGradId
  ...
  guardarFechaCantidadExamenCursos(curso, campoActualizar: string) {
    ...
    if (!curso.iCursoNivelGradId) {
      console.error('Error: Algunos cursos no contienen iCursoNivelGradId');
      this._MessageService.add({
        severity: 'error',
        summary: 'Error',
        detail: 'Algunos cursos no contienen iCursoNivelGradId.',
    ...
    if (campoActualizar === 'preguntas' && curso.iExamenCantidadPreguntas) {
      curso.iExamenCantidadPreguntas = parseInt(curso.iExamenCantidadPreguntas.toString(), 10);
    }

    if (campoActualizar === 'duracion' && curso.iExamenDuracionMinutos) {
      curso.iExamenDuracionMinutos = parseInt(curso.iExamenDuracionMinutos.toString(), 10);
    }
      ...
      .subscribe({
        next: () => {
          ...
          switch (campoActualizar) {
            case 'preguntas':
              detailMessage = 'Se ha actualizado la cantidad de preguntas.';
              break;
            case 'duracion':
              detailMessage = 'Se ha actualizado la duración de la evaluación.';
              break;
            case 'fecha':
              detailMessage = 'Se ha actualizado la fecha de inicio.';
              break;
            default:
              detailMessage = 'Se han actualizado los datos de la evaluación.';
              break;
          }
          this._MessageService.add({
            severity: 'success',
  ...
  habiltarButton(grados: any, curso: any): boolean {
    ...
    for (gradoKey of Object.keys(grados)) {
      for (item of grados[gradoKey]) {
        if (item.cCursoNombre === curso.cCursoNombre && item.isSelected) {
          return true;
        }
      }
    }
