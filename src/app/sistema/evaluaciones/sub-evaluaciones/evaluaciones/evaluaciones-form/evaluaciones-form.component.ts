...
export class EvaluacionesFormComponent implements OnInit {
  ...
  ereVerEvaluacion() {
    ...
    if (evaluacionData) {
      ...
      if (patchData.dtEvaluacionFechaInicio) {
        const [dia, mes, anio] = patchData.dtEvaluacionFechaInicio.split('/');
        patchData.dtEvaluacionFechaInicio = new Date(Number(anio), Number(mes) - 1, Number(dia));
      }
      if (patchData.dtEvaluacionFechaFin) {
        const [dia, mes, anio] = patchData.dtEvaluacionFechaFin.split('/');
        patchData.dtEvaluacionFechaFin = new Date(Number(anio), Number(mes) - 1, Number(dia));
      }
  ...
  guardarEvaluacion() {
    ...
    const data = {
      ...
      cEvaluacionNombre: this.evaluacionFormGroup.get('cEvaluacionNombre').value,
      cEvaluacionDescripcion: this.evaluacionFormGroup.get('cEvaluacionDescripcion').value,
      cEvaluacionUrlDrive: this.evaluacionFormGroup.get('cEvaluacionUrlDrive').value,
      dtEvaluacionFechaInicio: this.evaluacionFormGroup.get('dtEvaluacionFechaInicio').value,
      dtEvaluacionFechaFin: this.evaluacionFormGroup.get('dtEvaluacionFechaFin').value,
      iEstado: estado,
    };
    ...
    this._apiEre
      ...
      .subscribe({
        next: (resp: any) => {
          ...
          this.compartirIdEvaluacionService.iEvaluacionId = this.iEvaluacionId; // Guardar en el servicio

          const nombreEvaluacion = resp['data'][0]['cEvaluacionNombre']; // Obtiene el nombre de la respuesta
          this.compartirFormularioEvaluacionService.setcEvaluacionNombre(nombreEvaluacion);
          this._MessageService.add({
            severity: 'success',
  ...
  actualizarEvaluacion() {
    const fechaInicioOriginal = this.evaluacionFormGroup.get('dtEvaluacionFechaInicio').value;
    const fechaFinOriginal = this.evaluacionFormGroup.get('dtEvaluacionFechaFin').value;
    ...
    const data = {
      iEvaluacionId: Number(this.evaluacionFormGroup.get('iEvaluacionId').value),
      idTipoEvalId: Number(this.evaluacionFormGroup.get('idTipoEvalId').value),
      iNivelEvalId: Number(this.evaluacionFormGroup.get('iNivelEvalId').value),
      cEvaluacionNombre: this.evaluacionFormGroup.get('cEvaluacionNombre').value,
      cEvaluacionDescripcion: this.evaluacionFormGroup.get('cEvaluacionDescripcion').value,
      cEvaluacionUrlDrive: this.evaluacionFormGroup.get('cEvaluacionUrlDrive').value,
