...
export class EvaluacionesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.cEvaluacionNombre = this.compartirFormularioEvaluacionService.getcEvaluacionNombre();
    // Establece el ID de la evaluación
    this.compartirFormularioEvaluacionService.setEvaluacionId(this.iiEvaluacionId);
    this.compartirFormularioEvaluacionService.setcEvaluacionNombre(this.cEvaluacionNombre);
  ...
  async obtenerCursos() {
    ...
    if (!evaluacionCursos || !evaluacionCursos.cursos_niveles) {
      console.error('No se encontró la propiedad cursos_niveles en la respuesta');
      return;
    }
      ...
      nivel.grados.forEach(grado => {
        grado.cursos.forEach(curso => {
          this.form.addControl(
            ...
            new FormControl(curso.dtExamenFechaInicio ? new Date(curso.dtExamenFechaInicio) : null)
          );
          this.form.addControl(
            `${curso.cCursoNombre}${curso.iCursoNivelGradId}[${curso.iExamCurId}]Fin`,
            new FormControl(curso.dtExamenFechaFin ? new Date(curso.dtExamenFechaFin) : null)
          );
        });
      });
    });
      ...
      if (inicioValue || finValue) {
        coincidencias.push({
          ...
          campos: {
            ...
            dtExamenFechaInicio: this.utils.convertToSQLDateTime(inicioValue) || null, // Asigna el valor o null si no existe
            dtExamenFechaFin: this.utils.convertToSQLDateTime(finValue) || null, // Asigna el valor o null si no existe
          },
          where: {
    ...
    switch (accion) {
      ...
      case 'eliminar':
        this._confirmService.openConfirm({
          header: '¿Esta seguro de eliminar la evaluación: ' + item['cEvaluacionNombre'] + ' ?',
          accept: () => {
        ...
        break;
      case 'verListaAreas':
        this.compartirFormularioEvaluacionService.setcEvaluacionNombre(item.cEvaluacionNombre);
        this.router.navigate(['ere/evaluaciones/' + item.iEvaluacionIdxHash + '/areas']);
        break;
      case 'verListaExclusion':
        this.router.navigate(['ere/evaluaciones/' + item.iEvaluacionIdxHash + '/exclusiones']);
        break;
  ...
  onEvaluacionSeleccionada(event: any) {
    ...
    this.compartirFormularioEvaluacionService.setcEvaluacionNombre(this.nombreEvaluacion);
      ...
      .subscribe({
        next: (resp: unknown) => {
          ...
          if (resp && resp['data']) {
            ...
            console.warn('La respuesta no contiene la propiedad "data" o es nula:', resp);
          }
        },
        error: err => {
          // Manejo de errores
  ...
  agregarEditarPregunta(evaluacion) {
    ...
    if (accion === 'nuevo') {
      ...
      nombreEvaluacion = evaluacion?.cEvaluacionNombre || 'Editar Evaluación';
    }
