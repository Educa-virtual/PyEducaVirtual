...
@Component({
  ...
  standalone: true,
  imports: [TableModule, TablePrimengComponent, ButtonModule, ConfirmDialogModule, ToastModule],
...
export class DiasLaboralesComponent implements OnInit {
  ...
  confirm() {
    ...
    this.stepConfirmationService.confirmAction(
      {
        onAcceptPromises: [() => this.saveInformation(), () => this.nextPage()],
      },
      message
    );
  }
  ...
  async saveInformation() {
    ...
    if (Array.isArray(this.ticketService.registroInformation.stepDiasLaborales)) {
      ...
      ExistDiaNoSelection = this.ticketService.registroInformation.stepDiasLaborales.filter(
        diaExist => !this.diasSelections.some(dia => dia.iDiaId == diaExist.iDiaId)
      );
    } else {
    ...
    if (Array.isArray(noExistDiaSelection) && noExistDiaSelection.length > 0) {
      await this.ticketService.insCalDiasLaborales(noExistDiaSelection);
    }

    if (Array.isArray(ExistDiaNoSelection) && ExistDiaNoSelection.length > 0) {
