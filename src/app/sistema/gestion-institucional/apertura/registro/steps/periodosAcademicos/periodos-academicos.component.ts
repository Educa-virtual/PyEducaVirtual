...
export class PeriodosAcademicosComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.periodosAcademicos = (await this.ticketService.selPeriodosFormativos()).data;
  ...
  async setFasesPromocionales() {
    if (Array.isArray(this.ticketService.registroInformation.stepYear.fases_promocionales)) {
      this.fasesPromocionales = this.ticketService.registroInformation.stepYear.fases_promocionales
        ...
        .map((fase, index) => ({
          ...
          dtFaseInicioVisual: this.ticketService.toVisualFechasFormat(
            fase.dtFaseInicio,
            'DD/MM/YYYY'
          ),
          dtFaseInicio: fase.dtFaseInicio,
          dtFaseFin: fase.dtFaseFin,
          dtFaseFinVisual: this.ticketService.toVisualFechasFormat(fase.dtFaseFin, 'DD/MM/YYYY'),
        }));
    }
  }
  ...
  nextPage() {
    this.router.navigate(['gestion-institucional/apertura/registro/resumen']);
  }
  ...
  async saveInformation() {
    ...
    if (Array.isArray(this.ticketService.registroInformation.stepPeriodosAcademicos)) {
      periodo = this.ticketService.registroInformation.stepPeriodosAcademicos.filter(
  ...
  setPeriodoFormativo(periodo) {
    ...
    this.ciclosAcademicos = this.fechasCalculadas.map((fecha, index) => ({
      ...
      startDateVisual: this.ticketService.toVisualFechasFormat(fecha.inicio, 'DD/MM/YYYY'),
      startDate: fecha.inicio,
      endDateVisual: this.ticketService.toVisualFechasFormat(fecha.fin, 'DD/MM/YYYY'),
      endDate: fecha.fin,
    }));
  }
  ...
  setPeriodosFormativos() {
    ...
    if (Array.isArray(this.ticketService.registroInformation.stepPeriodosAcademicos)) {
      ...
      console.log(this.ticketService.registroInformation.stepPeriodosAcademicos);
  ...
  saveInformationEdit() {
    ...
    if (index !== -1) {
      this.ciclosAcademicos[index].startDate = new Date(this.formPeriodo.value.fechaInicio);
      this.ciclosAcademicos[index].startDateVisual = this.ticketService.toVisualFechasFormat(
        this.formPeriodo.value.fechaInicio,
        'DD/MM/YYYY'
      );
      this.ciclosAcademicos[index].endDateVisual = this.ticketService.toVisualFechasFormat(
        this.formPeriodo.value.fechaFin,
        'DD/MM/YYYY'
      );
      this.ciclosAcademicos[index].endDate = new Date(this.formPeriodo.value.fechaFin);
    }
