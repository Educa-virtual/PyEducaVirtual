...
export class ResumenComponent implements OnInit {
  ...
  nextPage() {
    this.router.navigate(['gestion-institucional/apertura/registro/periodos-academicos']);
  }
  ...
  ngOnInit() {
    ...
    this.resumenInformation.calendar = this.ticketService.registroInformation.stepYear;

    this.resumenInformation.diasLaborales = this.ticketService.registroInformation.stepDiasLaborales
      .map(dia => dia.cDiaNombre)
      .join(',');

    this.resumenInformation.formasAtencion =
      this.ticketService.registroInformation.stepFormasAtencion.map((formaAtencion, index) => ({
        index: index + 1,
    ...
    this.resumenInformation.periodosAcademicos =
      this.ticketService.registroInformation.stepYear.fases_promocionales.map(fase => {
        ...
        const periodosEvalForm = this.ticketService.registroInformation.stepPeriodosAcademicos
          .filter(periodo => periodo.iFaseId == fase.iFaseId)
          .map((periodo, index) => {
            let periodoType;
            console.log(periodo);
            ...
            return {
              ...
              dtPeriodoEvalAperInicio: this.ticketService.toVisualFechasFormat(
                periodo.dtPeriodoEvalAperInicio,
                'DD/MM/YYYY'
              ),
              dtPeriodoEvalAperFin: this.ticketService.toVisualFechasFormat(
