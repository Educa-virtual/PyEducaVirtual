...
export class YearComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    await this.ticketService.setCalendar(
      { iCalAcadId: '' },
      {
        onCompleteCallbacks: [data => this.setValuesFormCalendar(data)],
      }
    );

    const file = await this.ticketService.setReglamentoInterno();

    const fileObj = this.ticketService.base64ToFile(file.value, file.name, file.mimeType);
  ...
  setValuesFormCalendar(data) {
    ...
    if (this.ticketService.registroInformation.calendar.iCalAcadId) {
      ...
      if (Array.isArray(fases_promocionales)) {
        const idExistsFases = fases_promocionales.map(faseExist => String(faseExist.iFasePromId));
        ...
        console.log(this.ticketService.registroInformation.stepYear.fases_promocionales);

        this.ticketService.registroInformation.stepYear.fases_promocionales.forEach(fase => {
          ...
          this.form.addControl(
            ...
            new FormControl([new Date(fase.dtFaseInicio), new Date(fase.dtFaseFin)])
          );
        ...
        noExistsFases.forEach(fase => {
          this.form.addControl('faseCheck' + fase.iFasePromId, new FormControl([fase]));

          this.form.addControl('faseInputInicio' + fase.iFasePromId, new FormControl(''));

          this.form.addControl('faseInputFin' + fase.iFasePromId, new FormControl(''));
      ...
      this.form.patchValue({
        ...
        fechaMatriculaFin: (matriculaFin && [matriculaInicio, matriculaFin]) ?? '',
        fechaMatriculaRezagados: (matriculaResagados && [matriculaFin, matriculaResagados]) ?? '',
      });
    } else {
      this.form.patchValue({
        fechaVigente: data.yearAcad.cYAcadNombre,
        fechaInicio: [new Date(data.yearAcad.dtYAcadInicio), new Date(data.yearAcad.dYAcadFin)],
      });
  ...
  isActiveFechasFases(checkId = null) {
    if (checkId && this.form.get('faseCheck' + checkId).value.includes(true)) {
  ...
  async saveInformation() {
    ...
    const calFasesProm = this.filterInputsByPrefix(this.form.value, 'faseCheck').map(fase => ({
      iFasePromId: fase.iFasePromId,
      dtFaseInicio: fase.dtFaseInicio[0],
      dtFaseFin: fase.dtFaseInicio[1],
    }));

    const idCheckFases = calFasesProm.map(fase => Number(fase.iFasePromId));

    console.log('Fases Prom');
    console.log(idCheckFases);
    console.log(calFasesProm);

    if (Array.isArray(this.ticketService.registroInformation.stepYear.fases_promocionales)) {
      ...
      const idExistsFases = this.ticketService.registroInformation.stepYear.fases_promocionales.map(
        ...
        for (const [, Fase] of Object.entries(insCalFasesProm)) {
          this.apiService.updateData({
            ...
            where: {
              ...
              VALUE: this.ticketService.registroInformation.calendar.iCalAcadId,
            },
          });
        }
      ...
      if (existCalFasesProm.length > 0) {
        ...
        const updCalFasesProm =
          this.ticketService.registroInformation.stepYear.fases_promocionales.map(fase => {
            const obj = existCalFasesProm.find(
              existCalFaseProm => existCalFaseProm.iFasePromId == fase.iFasePromId
            );
  ...
  async nextPage() {
    this.router.navigate(['gestion-institucional/apertura/registro/dias-laborales']);
  }
}
