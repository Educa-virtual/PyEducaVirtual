      ...
      this.dialog.openConfirm({
        ...
        accept: () => {
          of(null)
            .pipe(
              switchMap(() =>
                this.nationalHolidayService.deleteFeriadosNacionalesXiFeriadoId(item)
              ),
              tap((res: any) => {
              ...
              switchMap(() => this.nationalHolidayService.getFeriadosNacionales())
            )
            .subscribe({
              next: (res: any) => {
                this.nationalHolyday.table.data.core = res.data.map(item => ({
                  ...item,
...
function saveData(this: ConfigFechasComponent) {
  ...
  if (!this.form.value.iFeriadoId) {
    of(null)
      .pipe(
        switchMap(() => this.nationalHolidayService.insFeriadosNacionales(data)),
        tap((res: any) => {
          const result = res.data[0];
          const isSuccess = result.Message === 'true';
    ...
    of(null)
      .pipe(
        switchMap(() => this.nationalHolidayService.updFeriadosNacionales(data)),
        tap((res: any) => {
          const result = res.data[0];
          const isSuccess = result.Message === 'true';
...
function importData(this: ConfigFechasComponent) {
  ...
  if (data.dataAccordingColumns.length > 0) {
    of(null)
      .pipe(
        switchMap(() =>
          this.nationalHolidayService.insFeriadosNacionalesMasivo(data.dataAccordingColumns)
        ),
        tap((res: any) => {
          this.messageService.add({
            severity: 'success',
