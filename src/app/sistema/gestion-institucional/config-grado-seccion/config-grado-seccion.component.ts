...
export class ConfigGradoSeccionComponent implements OnInit {
        ...
        this.config = [
          {
            ...
            cNivelTipoNombre: this.stepService.perfil['cNivelTipoNombre'],
        ...
        complete: () => {
          ...
          if (Number(this.sede[0].iYAcadId) === Number(this.dremoiYAcadId)) {
            this._confirmService.openAlert({
              ...
              message: 'El año escolar de I.E. es: ' + this.sede[0].cYAcadNombre,
  ...
  accionBtnItemTable({ accion, item }) {
    ...
    if (accion === 'agregar') {
      ...
      const resultado =
        this.configuracion_lista?.filter(item => item['cYAcadNombre'] === dremoYear) || [];

      if (resultado.length > 0) {
        this._confirmService.openAlert({
          header: 'Advertencia de configuracion',
          message: 'El año escolar de I.E. ' + this.sede[0].cYAcadNombre + ' ya existe',
          icon: 'pi pi-exclamation-triangle',
          //severity : "success"
        });
        this.messageService.add({
