...
export class ConfigAsignarGradoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    try {
      ...
      this.form = this.fb.group({
        ...
        iSeccionId: [{ value: 0, disabled: true }, [Validators.required]], //ok
  ...
  onChange(event: any, cbo: string): void {
    ...
    if (cbo === 'grado') {
      ...
      const filteredData = this.seccionesAsignadas.filter(item => item.iGradoId === selected);
  ...
  searchListarAsignaturas() {
    const iSeccionId = Number(this.formSearch.get('iSeccionId_search')?.value);
      ...
      .subscribe({
        ...
        complete: () => {
          ...
          if (iDocenteId > 0) {
            ...
            this.actualizarMinimo(this.form.get('iDocCursoHorasLectivas')?.value);
          } else {
  ...
  addAreaDocente() {
    ...
    const params = JSON.stringify({
      ...
      iDocCursoHorasLectivas: this.form.get('iDocCursoHorasLectivas')?.value,
      iEstado: 1,
  ...
  accionBtnItemTable({ accion, item }) {
    ...
    if (accion === 'editar') {
      ...
      this.form.get('iGradoId')?.setValue(this.formSearch.get('iGradoId')?.value);
    ...
    if (accion === 'agregar') {
      ...
      this.form.get('iSeccionId')?.setValue(this.formSearch.get('iSeccionId_search')?.value);
      this.form.get('iCursoId')?.setValue(item.iCursoId);
      this.form.get('iDocCursoHorasLectivas')?.setValue(item.nCursoTotalHoras); //.iDocCursoHorasLectivas
    }
    if (accion === 'eliminar') {
      this._confirmService.openConfirm({
