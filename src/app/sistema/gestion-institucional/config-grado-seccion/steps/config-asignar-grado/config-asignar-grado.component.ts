...
export class ConfigAsignarGradoComponent implements OnInit {
  ...
  onChange(event: any, cbo: string): void {
    ...
    const iNivelGradoId_search = this.formSearch.get('iNivelGradoId_search')?.value ?? null;
    const iModalServId_search = this.formSearch.get('iModalServId_search')?.value ?? null;
    ...
    if (cbo === 'modalidad') {
      ...
      this.cursos = []; // limpia los valores del array en caso de seleccion de cbo
      const filteredData = this.lista_niveles_grados.filter(
        item => item.iNivelGradoId === iNivelGradoId_search && item.iModalServId === selected
      );
      console.log(this.lista_niveles_grados, 'lista_niveles_grados modalidad');
      ...
      // Ordenar el arreglo sin importar el tamaño
      this.turnos = this.turnos.sort((a, b) => Number(a.iTurnoId) - Number(b.iTurnoId));
    ...
    if (cbo === 'turno') {
      ...
      // Ordenar el arreglo sin importar el tamaño
      this.secciones = this.secciones.sort((a, b) => Number(a.iSeccionId) - Number(b.iSeccionId));
  ...
  searchListarAsignaturas() {
    const params = JSON.stringify({
      iNivelGradoId: this.formSearch.get('iNivelGradoId_search')?.value ?? 0,
      iSemAcadId: this.formSearch.get('iSemAcadId_search')?.value ?? 0,
      iModalServId: this.formSearch.get('iModalServId_search')?.value ?? 0,
    ...
    this.query
      ...
      .subscribe({
        next: (data: any) => {
          ...
          console.log(this.lista_niveles_grados, 'this.lista_niveles_grados');
        },
        error: error => {
        ...
        complete: () => {
          ...
          this.grados = this.grados.sort(
            (a, b) => Number(a.iNivelGradoId) - Number(b.iNivelGradoId)
          );
  ...
  addAreaDocente() {
    ...
    const params = JSON.stringify({
      ...
      iCursosNivelGradId: Number(this.form.get('iCursosNivelGradId')?.value),
      iCursosNivelGradId_ies_cursos:
        Number(this.form.get('iCursosNivelGradId_ies_cursos')?.value) ?? 0,
      ...
      cDocCursoObservaciones: this.form.get('cDocCursoObservaciones')?.value,
  ...
  accionBtnItemTable({ accion, item }) {
    if (accion === 'editar') {
      ...
      this.form.get('iNivelGradoId')?.setValue(this.formSearch.get('iNivelGradoId_search')?.value);
      this.form.get('iModalServId')?.setValue(this.formSearch.get('iModalServId_search')?.value);
      this.form.get('iSemAcadId')?.setValue(this.formSearch.get('iSemAcadId_search')?.value);
      this.form.get('iTurnoId')?.setValue(this.formSearch.get('iTurnoId_search')?.value);
      this.form.get('iSeccionId')?.setValue(this.formSearch.get('iSeccionId_search')?.value);
      ...
      this.form.get('idDocCursoId')?.setValue(item.idDocCursoId);
      this.form.get('iCursosNivelGradId')?.setValue(item.iCursosNivelGradId);
      this.form.get('iCursosNivelGradId_ies_cursos')?.setValue(item.iCursosNivelGradId_ies_cursos);
