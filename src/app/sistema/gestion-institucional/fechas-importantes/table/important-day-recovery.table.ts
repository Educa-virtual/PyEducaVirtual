    ...
    case 'eliminar':
      this.dialog.openConfirm({
        ...
        accept: () => {
          of(null)
            .pipe(
              switchMap(() => this.importantDayService.deleteFechasImportantes(item)),
              tap((res: any) => {
              ...
              switchMap(() =>
                this.importantDayService.getDependenciaFechas(this.forms.importantDay.value)
              )
            )
            .subscribe({
              next: (res: any) => {
                ...
                if (iDepFechaImpId) {
                  this.importantDay.table.data.recovery = res.data.map(item => ({
                    ...item,
              ...
              error: error => {
                this.messageService.add({
                  severity: 'error',
                  summary: 'Fechas importantes',
                  detail: error ?? 'Ha ocurrido un error al eliminar el fecha importante',
                  life: 3000,
                });
              },
            });
...
function saveData(this: FechasImportentesComponent) {
  ...
  const data: any = {
    ...
    bFechaImpSeraLaborable: Number(this.forms.importantDayRecovery.value.bFechaImpSeraLaborable),
