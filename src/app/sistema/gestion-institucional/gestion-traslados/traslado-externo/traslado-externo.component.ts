...
export class TrasladoExternoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    try {
      this.form = this.fb.group({
        ...
        cDocumentoEstudiante: [{ value: null, disabled: true }, Validators.required],
        cDocumento: [null, Validators.required],
        iYearId: [{ value: 0, disabled: true }, Validators.required],
        cUltimoGradoEstudio: [{ value: 0, disabled: true }, Validators.required],
  ...
  validarDNI() {
    ...
    if (!cDocumentoEstudiante) {
      ...
      this.query.obtenerInformacionEstudianteDNI({ dni: cDocumentoEstudiante }).subscribe({
        next: (data: any) => {
        ...
        complete: () => {
          ...
          const registro = JSON.parse(this.estudiante[0].matriculas);
          ...
          this.matriculas = registro.map((m: any) => ({
            ...
            cMatrConclusionDescriptiva: m.cMatrConclusionDescriptiva,
          }));
          this.getNivelTipoId(Number(this.matriculas[0].iNivelTipoId), 'institucion');

          this.form.controls['iIieeId']?.setValue(this.matriculas[0].iIieeId.toString());
          this.form.controls['iYearId']?.setValue(this.matriculas[0].iYearId.toString());
  ...
  confirm() {
    ...
    if (!dni || dni.trim() === '') {
      ...
      const menssage =
        '¿Estás seguro de que deseas validar informacíon del estudiante ' + dni + '?';
