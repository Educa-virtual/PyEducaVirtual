...
export class ConfiguracionHorarioComponent implements OnInit {
  ...
  onChange(event: any, cbo: string): void {
    ...
    if (cbo === 'configuracion') {
      const filteredData = this.configuracion.filter(item => item.iConfigId === selected);

      this.formSearch.get('iYAcadId_search').setValue(filteredData[0].iYAcadId);
    ...
    if (cbo === 'horario') {
      ...
      const filteredDataHorario = this.horarios.filter(item => item.iHorarioIeId === selected);
        ...
        complete: () => {
          ...
          if (this.configuracion.length > 0) {
            ...
            this._confirmService.openAlert({
              ...
              message:
                'No se encuentra registro de ambientes no creado, año acdémico ' + this.dremoYear,
  ...
  procesarBloques(data: any) {
    this.items = data.flatMap((item: any) => {
      ...
      const horariosArray =
        typeof item.det_horario === 'string' ? JSON.parse(item.det_horario) : item.det_horario;

      if (!Array.isArray(horariosArray)) {
        console.error('Horarios no es un arreglo válido:', horariosArray);
  ...
  searchHorarioAreas(asignados: any) {
    ...
    this.areas_asignadas = asignados.flatMap((item: any) => {
      // Asegúrate de que 'horarios' sea un arreglo, parseando si es una cadena JSON
      const horariosArray =
        typeof item.horarios === 'string' ? JSON.parse(item.horarios) : item.horarios;
