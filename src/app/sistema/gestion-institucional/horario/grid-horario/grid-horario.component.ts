...
export class GridHorarioComponent implements OnChanges {
  ...
  procesarRegistros() {
    ...
    this.bloque_horario.forEach(hora => {
      ...
      while (actual < fin) {
        const siguiente = new Date(actual.getTime() + 45 * 60 * 1000);
        const bloqueInicio = this.formatearHora(actual);
        const bloqueFin = this.formatearHora(siguiente > fin ? fin : siguiente);
        intervalos.add(`${bloqueInicio}-${bloqueFin}`);
        actual = siguiente;
      }
    });
    ...
    this.bloques = Array.from(intervalos).sort((a, b) => {
      const [horaA] = a.split('-');
      const [horaB] = b.split('-');
      return this.convertirHoraAFecha(horaA).getTime() - this.convertirHoraAFecha(horaB).getTime();
    });
    ...
    this.bloques.forEach(bloque => {
      ...
      this.dias.forEach(dia => {
        const registro = this.horarios.find(
          r => r.dia === dia && this.estaDentroDelRango(inicio, fin, r.inicio, r.fin)
        );
        this.horario[bloque][dia] = registro
          ? {
              asignatura: registro.asignatura,
  ...
  estaDentroDelRango(inicio: string, fin: string, rangoInicio: string, rangoFin: string): boolean {
