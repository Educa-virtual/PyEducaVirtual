...
export class MantenimientoAddUserComponent implements OnChanges, OnInit {
  ...
  @Output() modal = new EventEmitter(); // emite usuario para registar
  ...
  accionBtn(event: any, accion: string) {
    ...
    if (accion === 'Validar') {
      if (event == 1) {
        ...
        this.form_user.get('cDocumento')?.setValidators([Validators.required]);
      }
        ...
        .subscribe({
          next: (data: any) => {
            ...
            this.messageService.add({
              ...
              detail: 'Credencial asignada correctamente',
            });
          },
          error: () => {
            this.messageService.add({
              severity: 'error',
              summary: 'Mensaje del sistema',
              detail: 'El usuario cuenta con credenciales activas',
            });
          },
          complete: () => {
