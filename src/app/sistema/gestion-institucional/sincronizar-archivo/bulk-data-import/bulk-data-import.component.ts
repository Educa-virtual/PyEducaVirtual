...
export class BulkDataImportComponent implements OnInit {
  ...
  constructor(
    ...
    this.dropdownConfigs.forEach(dropdown => {
      this.typeCollectionForm.addControl(`${dropdown.label}${dropdown.id}`, new FormControl(''));
    });
  }
  ...
  ngOnInit(): void {
    this.dropdownConfigs.forEach(config => {
      if (config.dependency) {
        ...
        this.typeCollectionForm.get(parentControlName)?.valueChanges.subscribe(() => {
          ...
          (this.bulkDataImport.importEndPoint = ''), (this.bulkDataImport.params = {});
    ...
    this.typeCollectionForm.valueChanges.subscribe(value => {
      ...
      this.isDisabled.downloadTemplate = !this.isSelectedTypeCollection(value);
      this.loadCollectionTemplate();
    });
  }

  isSelectedTypeCollection(obj: Record<string, any>): boolean {
    return Object.keys(obj).some(key => /Tipo de plantilla:\d+/i.test(key) && obj[key]);
  }
  ...
  shouldShowDropdown(config): boolean {
    ...
    return (
      this.typeCollectionForm.get(dependentDropdown.label + dependentDropdown.id)?.value.id ===
      config.optionValue
    );
  }

  loadCollectionTemplate() {
    const foundEntry: [any, any] = Object.entries(this.typeCollectionForm.value).find(
      ([key, value]) => key.startsWith('Tipo de plantilla:') && value !== null
    );
  ...
  validaImportData() {
    ...
    const fileImport = () => {
      switch (this.collection.typeSend) {
        case 'file':
          return this.file;
        case 'json':
          SheetToMatrix.getInstance('hojaDeDatosAImportar').setDataAccordingColumns();
          return undefined;
        default:
          return undefined;
      }
    ...
    this.bulkDataImport
      .importDataCollection(
        fileImport(),
        SheetToMatrix.getInstance('hojaDeDatosAImportar').dataAccordingColumns
      )
      .subscribe({
        next: response => {
          this.importLoad = false;
          this.responseDataImport = this.collection.response(response) ?? undefined;
        },
        error: error => {
          this.messageService.add({
            severity: 'error',
            summary: 'Error al importar archivo',
            detail: error.error.message ?? 'Ha ocurrido un error al importar los datos',
            life: 3000,
          });
