...
export class SheetToMatrix {
  ...
  private assignTableData() {
    for (const { columns, data, header, inTableColumn } of this.options.structures) {
      const rowDataGroup = [];
      const structureDataAddress = XLSX.utils.decode_range(data);
      const structureHeaderAddress = XLSX.utils.decode_range(header);

      columns.inTableColumnsGroup = this.assignColWithSpan(columns?.inTableColumnsGroup);

      columns.inTableColumns = columns?.inTableColumns?.map((column, index) => ({
        ...column,
        col: index + 1,
      }));
      ...
      for (const [cellName, any] of Object.entries(this.worksheet)) {
        ...
        if (columns.inTableColumns) {
          for (const { field } of columns.inTableColumns) {
            ...
            const locationCellColumn = XLSX.utils.decode_cell(cellNameColumn);

            const InColum = locationCellColumn.c === locationCellName.c;
            ...
            if (isRange(data)) {
              const dataInRange =
                structureDataAddress.s.c <= locationCellName.c &&
                structureDataAddress.s.r <= locationCellName.r &&
                structureDataAddress.e.c >= locationCellName.c &&
    ...
    const workbook: XLSX.WorkBook = {
      ...
      Sheets: worksheets.reduce(
        (acc, ws) => {
          ...
          const orderedQuestions = Array.from(allQuestions).sort((a, b) => a - b);
          ...
          const mappedData = ws.data.map((row, rowIdx) => {
            ...
            extendedColumns.forEach((col, colIdx) => {
              if (col.key === 'index') {
                ...
                const respuesta = respuestas.find((r: any) => r.p == questionNum);
                ...
                if (col.operation === 'sum') {
                  const total = col.key.reduce((acc, value) => {
          ...
          headers.forEach((header, colIdx) => {
            ...
            for (let rowIdx = 1; rowIdx <= mappedData.length; rowIdx++) {
