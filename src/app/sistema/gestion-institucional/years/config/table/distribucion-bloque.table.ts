    ...
    case 'ver':
      ...
      this.forms.distribucionBloque.patchValue({
        ...
        dtInicioBloque: new Date(
          (() => {
            const [day, month, year] = item.dtInicioBloque.split('/');
      ...
      this.dialogConfirm.openConfirm({
        ...
        accept: () => {
          of(null)
            .pipe(
              switchMap(() =>
                this.distribucionBloquesService.deleteDistribucionBloque(item.iDistribucionBloqueId)
              ),
              tap((res: any) => {
...
function saveData(this: YearsComponent) {
  ...
  if (!this.forms.distribucionBloque.value.iDistribucionBloqueId) {
    of(null)
      .pipe(
        switchMap(() => this.distribucionBloquesService.insDistribucionBloques(data)),
        tap((res: any) => {
          const result = res.data[0];
    ...
    data.iDistribucionBloqueId = this.forms.distribucionBloque.value.iDistribucionBloqueId;

    of(null)
      .pipe(
        switchMap(() => this.distribucionBloquesService.updDistribucionBloque(data)),
        tap((res: any) => {
          const result = res.data[0];
          const isSuccess = result.Message === 'true';
